...
if(section){
  ...
  media.add('(max-width:700px), (max-width:1000px) and (max-height:500px)',()=>{
    ...
    const clones=columns.flatMap(column=>Array.from(column.children).map(child=>{
      const clone=child.cloneNode(true) as HTMLElement;clone.setAttribute('aria-hidden','true');clone.removeAttribute('data-reveal');column.append(clone);return clone;
    }));
    ...
    return()=>{observer.disconnect();tweens.forEach(t=>t.kill());clones.forEach(c=>c.remove());columns.forEach(c=>c.style.removeProperty('transform'));section.removeAttribute('data-mobile-streams');};
  });
